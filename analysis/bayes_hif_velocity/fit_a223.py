...
def _stitch() -> None:
    ...
    lines = [
        ...
        "",
        "`hypoxia_plus` is Image-iT+ in the GFP channel, **the same probe as E15S** (not transgenic GFP, not ROS/DCF). Fits are **per chemistry** (E27 3′, E29 5′). Lineage comes from the tumor and neutrophil QC tables, not from every droplet in the OCM object (E27 has 731 DN droplets; only 13 are QC tumors). DN is Image-iT−; hypoxia_plus / DP / lactate_plus are exposed. The dye is an oxygen reporter, not HIF-α protein. Do not transfer the E14 control location. Do not write into A223 h5ads.",
        "",
        "DN n is below 40 in every lineage. Control MAD for \(h\) and the control mean of \(\\xi\) are poorly identified. Forced DN persist is 0% by design. Neutrophil persist on E29 (~93% of exposed) is what a 3-cell control MAD does — not a HIF-target call. E27 vs E29 tumor persist (39% vs 7%) is not replicated across chemistry; UMI ratio is 0.52 vs 2.75. Lag \(p^{\\mathrm{away}}\) is ~0.01–0.02 in tumors for DN and Image-iT+ alike.",
        "",
        "## Per-gate fractions",
        "",
        body,
