#!/usr/bin/env python3
"""Single-cohort HIF-α lag fit (one sample, tumors only)."""
...
def run_cohort(
    ...
    *,
    ...
    cells.to_csv(out / f"{slug}_tumor_cells.tsv", sep="\t", index=False)
    genes.to_csv(out / f"{slug}_tumor_genes.tsv", sep="\t", index=False)
    pd.DataFrame(cycle_rows).to_csv(out / f"{slug}_tumor_cycle.tsv", sep="\t", index=False)
    ...
    write_md(out / f"{slug}_tumors.md", lines)
    print("wrote", out / f"{slug}_tumors.md", flush=True)
